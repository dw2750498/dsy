...
public class TicketServlet extends HttpServlet
{
    ...
    private Map<Integer, Ticket> ticketDatabase = new LinkedHashMap<>();//用哈希表存储数据

    @Override
    /*doget方法使用原始操作/执行器模式：
    操作通过请求参数传入，doget方法根据操作将请求发送给执行器（方法）*/
    ...
    {
        String action = request.getParameter("action");
        if(action == null)  //重定向方法：如果客户端执行了一个不含action参数或者含有无效action参数的POST请求，
                            // 浏览器页面将被重定向至显示票据的页面
            action = "list";
        switch(action)
        {
    ...
    //该方法将使用processAttachment方法和其他请求参数填充ticket对象，并将该对象添加到数据库中
            ...
            throws ServletException, IOException
    {
        ...
        {
            Attachment attachment = this.processAttachment(filePart);//processAttachment方法负责文件上传任务
        ...
        int id;
        synchronized(this)//使用synchronized关键字保护共享资源，保证多线程执行时正确
        {
  ...
  //该方法将从mulitipart请求中获得inputstream，并将它复制到Attachment对象中。
